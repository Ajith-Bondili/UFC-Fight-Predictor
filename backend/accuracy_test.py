...
def main():
    ...
    def enough_bouts(row):
        return counts.get(row["fighter_1"], 0) >= 1 and counts.get(row["fighter_2"], 0) >= 1

    filtered_test = test_df[test_df.apply(enough_bouts, axis=1)]
    print(f"After filtering for ≥1 prior fights per fighter: {len(filtered_test)} test fights\n")
    ...
    print(f"Tested {len(results)} fights (UFC ≥308).")
